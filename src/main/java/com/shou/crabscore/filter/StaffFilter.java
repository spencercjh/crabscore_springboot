...
@Log4j2
@WebFilter(urlPatterns = "/api/staff/*", filterName = "工作人员用户组")
public class StaffFilter implements Filter {
    private final SecurityService securityService;

    @Autowired
    public StaffFilter(SecurityService securityService) {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) {
        ...
        if (!this.securityService.verify(jwt, CommonConstant.USER_TYPE_STAFF, request).getCode().equals(CommonConstant.SUCCESS)) {
            try {
