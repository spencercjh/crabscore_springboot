...
@Log4j2
@WebFilter(urlPatterns = "/api/judge/*", filterName = "评委用户组")
public class JudgeFilter implements Filter {
    private final SecurityService securityService;

    @Autowired
    public JudgeFilter(SecurityService securityService) {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) {
        ...
        if (!this.securityService.verify(jwt, CommonConstant.USER_TYPE_JUDGE, request).getCode().equals(CommonConstant.SUCCESS)) {
            try {
